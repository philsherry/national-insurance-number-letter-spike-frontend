...
class WhatIsYourTelephoneNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  "WhatIsYourTelephoneNumber Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, whatIsYourTelephoneNumberRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[WhatIsYourTelephoneNumberView]
