...
class WhenDidYouGetMarriedControllerSpec extends SpecBase with MockitoSugar {

  val formProvider = new WhenDidYouGetMarriedFormProvider()
  ...
  lazy val whenDidYouGetMarriedRoute = routes.WhenDidYouGetMarriedController.onPageLoad(NormalMode).url

  override val emptyUserAnswers = UserAnswers(userAnswersId)

  def getRequest(): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, whenDidYouGetMarriedRoute)

  def postRequest(): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, whenDidYouGetMarriedRoute)
  ...
  "WhenDidYouGetMarried Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(WhenDidYouGetMarriedPage, validAnswer).success.value
