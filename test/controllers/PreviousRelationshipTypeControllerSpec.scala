...
class PreviousRelationshipTypeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "PreviousRelationshipType Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, previousRelationshipTypeRoute)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, previousRelationshipTypeRoute)
            .withFormUrlEncodedBody(("value", PreviousRelationshipType.values.head.toString))
