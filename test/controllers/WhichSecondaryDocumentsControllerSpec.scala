...
class WhichSecondaryDocumentsControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  lazy val whichSecondaryDocumentsRoute = routes.WhichSecondaryDocumentsController.onPageLoad(NormalMode).url

  val formProvider = new WhichSecondaryDocumentsFormProvider()
  val form = formProvider()

  "WhichSecondaryDocuments Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, whichSecondaryDocumentsRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[WhichSecondaryDocumentsView]
