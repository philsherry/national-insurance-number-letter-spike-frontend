...
object WhatIsYourCurrentAddressInternationalSummary  {
    ...
    answers.get(WhatIsYourCurrentAddressInternationalPage).map {
      answer =>
        ...
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.WhatIsYourCurrentAddressInternationalController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("whatIsYourCurrentAddressInternational.change.hidden"))
          )
        )
    }
}
