...
sealed trait PreviousRelationshipType
...
object PreviousRelationshipType extends Enumerable.Implicits {
  ...
  def options(implicit messages: Messages): Seq[RadioItem] = values.zipWithIndex.map {
    case (value, index) =>
      RadioItem(
        content = Text(messages(s"previousRelationshipType.${value.toString}")),
        value   = Some(value.toString),
        id      = Some(s"value_$index")
      )
  }

  implicit val enumerable: Enumerable[PreviousRelationshipType] =
